...
from satellite_terrestrial.User_Init import *
import math as m
...
mm = MinMaxScaler()
train_data = []#储存训练数据
train_data_label = []#储存对应标签
for num in range(2000, 2500, 1):#for循环2000 ~ 2499
    action = []
...
for epoch in range(EPOCH):
    for step, (x, b_label) in enumerate(train_loader):
        ...
        gain_selection = power_distribution * 20 + 35#(35-55)
        power_selection1 = 68 - gain_selection - power_distribution * 30
        power_W = torch.pow(10, power_selection1 / 10)   #功率
